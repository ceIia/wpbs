...
describe("URL handling in WYSIWYG contexts", () => {
  ...
  it("should replace www URLs in admin page", async () => {
    const response = await fetch(`${ADMIN_URL}/post.php?post=123&action=edit`);
    const html = await response.text();
    // Test ajaxurl variable
    expect(html).toContain(
      'var ajaxurl = \'https://wp.some-domain.com/wp-admin/admin-ajax.php\';'
    );
    
    // Test wp-api-fetch configuration
    expect(html).toContain(
      'wp.apiFetch.nonceEndpoint = "https://wp.some-domain.com/wp-admin/admin-ajax.php?action=rest-nonce"'
    );

    // Test VAA_View_Admin_As configuration
    expect(html).toContain(
      '"ajaxurl":"https://wp.some-domain.com/wp-admin/admin-ajax.php"'
    );
  });
